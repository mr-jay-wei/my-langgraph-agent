...
#如果要使用mcp server,先把prox关闭，mcp_server.py跑起来：python mcp_server.py
...
# 导入MCP工具适配器
from mcp_tools_adapter import get_mcp_tools_as_langchain
...
if __name__ == "__main__":
    ...
    # 命令行参数解析
    parser = argparse.ArgumentParser(description='启动ReAct Agent，可选择是否集成MCP工具')
    parser.add_argument('--mcp', 
                      default='http://127.0.0.1:8083/my-custom-path',
                      help='主MCP服务器URL，默认为 http://127.0.0.1:8083/my-custom-path')
    parser.add_argument('--mcp2', 
                      default='http://127.0.0.1:8086/my-custom-path',
                      help='第二个MCP服务器URL，默认为 http://127.0.0.1:8086/my-custom-path')
    ...
    if not args.no_mcp:
        ...
        if args.mcp:
            try:
                ...
                mcp_tools = get_mcp_tools_as_langchain(args.mcp)
                print(f"成功从主MCP服务器加载 {len(mcp_tools)} 个工具:")
                for tool in mcp_tools:
                    print(f"- {tool.name}: {tool.description}")
                # 将MCP工具添加到工具列表
                tools_list.extend(mcp_tools)
            except Exception as e:
                print(f"加载主MCP服务器工具失败: {e}")
        ...
        if args.mcp2:
            try:
                ...
                mcp2_tools = get_mcp_tools_as_langchain(args.mcp2)
                print(f"成功从第二个MCP服务器加载 {len(mcp2_tools)} 个工具:")
                for tool in mcp2_tools:
                    print(f"- {tool.name}: {tool.description}")
                # 将MCP工具添加到工具列表
                tools_list.extend(mcp2_tools)
            except Exception as e:
                print(f"加载第二个MCP服务器工具失败: {e}")
                print("将继续使用已加载的工具...")
    else:
